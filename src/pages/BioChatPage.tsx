...
import { Send, Trash2, Loader2, Plus, MessageSquare, ChevronLeft, ChevronRight, MoreHorizontal, Pencil, Trash, X, Check } from 'lucide-react';
...
const SUGGESTIONS = [
  ...
  '🎯 Bio para uma nutricionista especialista em emagrecimento',
  '💼 Bio para um consultor de negócios digitais',
  '📸 Bio para um fotógrafo de casamentos',
  '🛍️ Bio para uma loja de roupas femininas online',
];
...
export default function BioChatPage() {
  ...
  return (
    <div className="flex h-screen w-full flex-col overflow-hidden bg-background">
      <StudioTopbar title="Bio Master" showApiKey={true} />
      <div className="flex flex-1 overflow-hidden">
        {/* Sidebar */}
        <div className={cn('flex flex-col border-r border-border/15 bg-card/30 backdrop-blur-sm transition-all duration-300 shrink-0', sidebarOpen ? 'w-64' : 'w-0 overflow-hidden')}>
          ...
          <ScrollArea className="flex-1">
            <div className="p-2 space-y-0.5">
              {conversations.length === 0 ? (
                ...
                <div key={convo.id} className={cn('group flex items-center gap-2 rounded-lg px-3 py-2.5 cursor-pointer transition-all duration-150', activeConvoId === convo.id ? 'bg-primary/10 text-foreground' : 'hover:bg-secondary/40 text-muted-foreground hover:text-foreground')} onClick={() => handleSelectConvo(convo.id)}>
                  <MessageSquare className="h-3.5 w-3.5 shrink-0 opacity-50" />
                  <div className="flex-1 min-w-0">
                    {editingId === convo.id ? (
                      <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
                        <input type="text" value={editTitle} onChange={(e) => setEditTitle(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') handleRenameConvo(convo.id, editTitle); if (e.key === 'Escape') setEditingId(null); }} className="flex-1 bg-transparent border-b border-primary/30 text-[11px] outline-none py-0.5" autoFocus />
                  ...
                  {editingId !== convo.id && (
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-36">
                        ...
                        <DropdownMenuItem className="text-destructive focus:text-destructive" onClick={(e) => { e.stopPropagation(); handleDeleteConvo(convo.id); }}><Trash className="h-3 w-3 mr-2" /> Excluir</DropdownMenuItem>
                      </DropdownMenuContent>
                    </DropdownMenu>
                  )}
                </div>
        ...
        {/* Sidebar toggle */}
        <button onClick={() => setSidebarOpen(!sidebarOpen)} className="absolute left-0 top-1/2 -translate-y-1/2 z-20 bg-card/60 border border-border/15 rounded-r-lg p-1.5 hover:bg-card/80 transition-all" style={{ left: sidebarOpen ? '256px' : '0px', transition: 'left 0.3s' }}>
        ...
        <div className="flex flex-1 flex-col overflow-hidden">
            ...
            {messages.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full min-h-[60vh] gap-6 animate-fade-up">
                ...
                <div className="text-center space-y-2">
                  <h2 className="text-2xl font-bold font-display text-foreground">Bio Master</h2>
                  <p className="text-sm text-muted-foreground max-w-md">Especialista em biografias estratégicas para Instagram. Me conte sobre você ou sua marca e eu crio 3 opções de bio.</p>
                </div>
                <div className="flex flex-wrap gap-2 max-w-xl justify-center">
                  {SUGGESTIONS.map((s, i) => (
                    <button key={i} onClick={() => send(s)} className="rounded-xl border border-border/30 bg-card/50 px-3 py-2 text-xs text-muted-foreground hover:text-foreground hover:border-primary/30 hover:bg-card/80 transition-all duration-200 text-left">{s}</button>
                  ))}
                </div>
              </div>
            ) : (
              <div className="max-w-3xl mx-auto space-y-6">
                ...
                {isLoading && messages[messages.length - 1]?.role !== 'assistant' && (
                  <div className="flex gap-3 justify-start">
                    <div className="shrink-0 w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center text-lg">✍️</div>
          ...
          <div className="border-t border-border/15 bg-card/20 backdrop-blur-sm p-3 md:p-4">
            <div className="max-w-3xl mx-auto flex gap-2">
              <Textarea ref={textareaRef} value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={handleKeyDown} placeholder="Me conte sobre você, sua marca e seu nicho..." className="flex-1 min-h-[44px] max-h-32 resize-none rounded-xl border-border/20 bg-secondary/30 text-sm placeholder:text-muted-foreground/40 focus:border-primary/30 focus:ring-primary/20" rows={1} />
              <div className="flex flex-col gap-1.5">
                <Button onClick={() => send()} disabled={isLoading || !input.trim()} size="icon" className="h-[44px] w-[44px] rounded-xl bg-primary hover:bg-primary/90 shadow-[0_0_15px_hsl(var(--primary)/0.3)] disabled:opacity-30 disabled:shadow-none">
                  {isLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}
                </Button>
                {messages.length > 0 && (
                  <Button onClick={clearChat} size="icon" variant="ghost" className="h-7 w-[44px] rounded-lg text-muted-foreground/40 hover:text-destructive"><Trash2 className="h-3 w-3" /></Button>
                )}
              </div>
            </div>
          </div>
